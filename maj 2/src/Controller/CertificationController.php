...
class CertificationController extends AbstractController
{
    ...
    public function create (Request $request, EntityManagerInterface $em): Response
     {
        $this->denyAccessUnlessGranted("IS_AUTHENTICATED_FULLY");
        $ide = $this->getUser()->getEntreprise()->getId(); // Récupération de l'ID de l'entreprise
        $certification = new Certification();
        $form = $this->createForm(CertificationType::class, $certification,['ide' => $ide]);
        $form->handleRequest($request);
    
        if ($form->isSubmitted() && $form->isValid()) {
            $instruments = $form['instrument']->getData();
            $codeinstrument = $instruments->getCodeinstrument();
            $certification->setIdentreprise($ide);
     ...
     {  
        ...
        $a = [];
                    ...
                    // $test = ($marge <= $tolerance && $marge >= -$tolerance) ? "oui" : "non";

    foreach ($ctrlfidelite as $key => $controlefidelite) {
        $a[$key] = $controlefidelite->getFidelitecorrecte();    
        }
        if (in_array('non',$a)){
        ...
        $b[$key] = $controledexcentration->getExcentrationcorrecte ();    
        }
        if (in_array('non',$b)){
        ...
        $c[$key] = $controlejustesse->getJustessecorrecte();    
            }
            if (in_array('non',$c)){
        ...
        $logoPath = $package->getUrl('uploads/logoentreprise/' . $entreprise->getLogo()) ? $package->getUrl('uploads/logoentreprise/' . $entreprise->getLogo()) : 0 ;
